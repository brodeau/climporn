...
requiredNamed.add_argument('-w', '--what', required=True, help='specify the field/diagnostic to plot (ex: CSPEED,CURLOF,SLINE,ect.)')
...
if   CWHAT == 'CURL':
    ...
    l_do_strl = True  ; # do current streamlines
    tmin=0. ; tmax=1.8 ; df = 0.2 ; cpal_fld = 'on3' ; # Poster full-res!!!
    cunit = 'Surface current speed [m/s]' ; cb_jump = 1
    cb_extend = 'max' ;#colorbar extrema
...
if not l_do_strl: Xplot = nmp.zeros((nj,ni))
...
for jt in range(jt0,Nt):
    ...
    if not l_do_strl: del lx, ly, XFLD, YFLD
    ...
    if l_do_strl:
        print 'STREAM!'
        #Y, X = nmp.mgrid[Xlon[:,0], Xlat[200,:]]
        #X, Y = nmp.meshgrid(Xlon[:,0], Xlat[200,:])
        X, Y = nmp.meshgrid(Xlon, Xlat)
        sys.exit(0)
        cf = ax.streamplot(Xlon, Ylat, XFLD, YFLD, color='g', linewidth=1, cmap=plt.cm.inferno, density=2, arrowstyle='->', arrowsize=1.5)
    else:
